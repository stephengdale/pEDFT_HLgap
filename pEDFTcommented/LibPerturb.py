...
class ExcitationHelper:
    def __init__(self, wfn, RKS=True, wfn0=None,
        ...
        if wfn.nirrep()>1:
            # It is very much recommended to never use this stuff by setting
            # the molecular symmetry to C1 - it doesn't seem to work at all...
            
            psi4.set_options({"reference":"rks"}) # This is a hack
            # Strip symmetries
            ...
            print("# ground state has %d > 1 symmetry groups - working on C1 deep copy"%(wfn.nirrep()))
            self.wfn = wfn.c1_deep_copy(c1_basis)
            
        if wfn0 is None:
            wfn0 = wfn
        
        self.Da= wfn0.Da().to_array(dense=True)
        self.epsilon = wfn0.epsilon_a().to_array(dense=True)
        self.C = wfn0.Ca().to_array(dense=True)
        self.F = wfn0.Fa().to_array(dense=True)
        ...
        if self.UseDegen:
            self.DegenHelp = DegenHelper(wfn)

            # Setting FreezeOrder to true ensures later eigenvalues are sorted
            # according to the original symmetry order
            self.epsilon, self.C = self.DegenHelp.eigh(self.F, b=self.S_ao,
                                                       FreezeOrder=True)
    ...
    def SolveGeneral(self, k0=None, k1=None,
        ...
        for step in range(MaxStep): # If it's not done in MaxStep it's probably oscillating
            ...
            if self.UseDegen:
                w,U = self.DegenHelp.eigh_p(FNew, CX)
            else:
            ...
            if MOM:
                iU2 = np.argmax(U**2,axis=0)
                i1 = np.abs(iU2-dk1).argmin()
            else: i1 = dk1
        ...
        if Err<ErrCut:
            if not(Silent):
                print("# Took %d steps to get to sc Err = %.6f"%(step, Err))
            ...
            if self.UseDegen:
                w,U = self.DegenHelp.eigh_p(FFinal, CX)
            else:
        ...
        # Oscillator strength (not working)
        Di01 = np.array([C0.dot(X).dot(C1) for X in self.Di_ao])
        self.f01 = 2/3*self.DE01*Di01**2
